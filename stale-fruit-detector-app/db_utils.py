...
MONGO_URI = "mongodb+srv://chinna4812:chinna1234@my-first-cluster.m1waocd.mongodb.net/"
...
def save_prediction(user_email, result, confidence, fruit_type, image_path=None, shelf_life_condition=None, shelf_life_estimate=None, shelf_life_confidence=None, model_type=None):
    """Save prediction to database - always create new entry"""
    try:
        # Always insert new prediction for each image analysis
        prediction_data = {
            ...
            "confidence": confidence  # Store confidence as well
        }
        predictions_collection.insert_one(prediction_data)
        logger.info(f"Inserted new prediction for user {user_email} with result: {result}")
    ...
    try:
        ...
        for pred in predictions:
            formatted_predictions.append((
                ...
                pred.get("model_type", "ViT"),  # Default to ViT for older entries
                pred.get("shelf_life_condition"),
                pred.get("shelf_life_estimate"),
                pred.get("image_path")
            ))
        
        logger.info(f"Retrieved {len(formatted_predictions)} predictions for user {user_email}")
