...
class DatabaseManager:
    ...
    def add_user(self, email, password):
        ...
        try:
            ...
            logger.error(f"DuplicateKeyError while adding user {email}: {str(e)}")
            return False
        except ConnectionFailure as e:
            logger.error(f"ConnectionFailure while adding user {email}: {str(e)}")
            return False
        except Exception as e:
            logger.error(f"Unexpected error while adding user {email}: {str(e)}")
            return False
